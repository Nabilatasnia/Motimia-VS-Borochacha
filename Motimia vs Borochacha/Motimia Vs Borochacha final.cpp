...
int startScreenbg,instructionbg,userinputbg,lastscreenbg, gamebg, creditbg,leaderboardbg;
int indexNumber=0;
bool musicOn=true;
...
void flagChecker()
{
    ...
    if(counter%17==0)
        jamilFlag=true;
    if((mm1X >= bcX-10 && mm1X <= bcX+128)&&(mm1Y >= bcY && mm1Y <= bcY+148))
        mm1Catch=true;
    if((mm2X >= bcX-10 && mm2X <= bcX+128)&&(mm2Y >= bcY && mm2Y <= bcY+148))
        mm2Catch=true;
}
void gameMode()
{
    if(game == 0)
    {
        ...
        if(musicOn==false)
            ...
            iShowBMPAlternativeSkipWhite(20,20,"soundOn.bmp");
    }
    if(game == 1)
    {
        //START GAME
    ...
    {
        //High Score
        iShowImage(0,0,screenWidth,screenHeight,leaderboardbg);
        iShowBMPAlternativeSkipWhite(30,650,"back.bmp");
        int ycord=490;
        iSetColor(0,0,0);
        ...
        for(i=0; i<4; i++)
        {
            ...
            _itoa(num,pointf,10);
            iText(680,ycord,pointf,GLUT_BITMAP_HELVETICA_18);
            ycord-=100;
        }
        fclose(f);
    }
...
void iMouse(int button, int state, int mx, int my)
{
    if(game == 0)
    {
        if((mx>=580 && mx <=736) && my>=330  && my<=390)
        {
        ...
        else if((mx>=580 && mx <=736) && my>=260  && my<=320)
        {
            if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
                game = 2;//instruction button
        }
        else if((mx>=580 && mx <=736) && my>=190  && my<=250)
        {
            if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
                game = 3;
            writeinFile(p);
        ...
        else if((mx>=580 && mx <=736) && my>=120  && my<=180)
        {
            if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
                game = 4;//credit
        }
        else if((mx>=580 && mx <=736) && my>=50  && my<=110)
        {
            if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
                exit(0);
        }
...
void mm1()
{
    if(game == 1)
    {
        if(mm1Flag==true)
        {
            ...
            else if((mm1X >= bcX-10 && mm1X <= bcX+128)&&(mm1Y >= bcY && mm1Y <= bcY+128))
            {
                mm1X=rand() % 780+250;
                mm1Y = 720;
                point++;
...
void mm2()
{
    if(game == 1)
    {
        if(mm2Flag==true)
        {
            ...
            else if((mm2X >= bcX-10 && mm2X <= bcX+128)&&(mm2Y >= bcY && mm2Y <= bcY+128))
            {
                mm2X=rand() % 780+250;
                mm2Y = 720;
                point++;
...
void ab()
{
    ...
    if(game == 1)
    {
        ...
        else if((abX >= bcX-10 && abX <= bcX+128)&&(abY >= bcY && abY <= bcY+128))
        {
            abX=rand() % 780+250;
            abY = 720;
            point--;
...
void inkRefill()
{
    if(game == 1)
    {
        if(inkFlag==true)
        {
            ...
            else if((inkX >= bcX-10 && inkX <= bcX+128)&&(inkY >= bcY && inkY <= bcY+128))
            {
                inkX=rand() % 780+250;
                inkY = 720;
                ink+=5;
...
void fmm()
{
    ...
    if(game == 1)
    {
        if(fmmFlag==true)
        {
            ...
            else if((fmmX >= bcX-10 && fmmX <= bcX+128)&&(fmmY >= bcY && fmmY <= bcY+128))
            {
                fmmX=rand() % 780+250;
                fmmY = 720;
                point+=5;
...
void kongka()
{
    if(game == 1)
    {
        if(kongkaFlag==true)
        {
            ...
            else if((kongkaX >= bcX-10 && kongkaX <= bcX+128)&&(kongkaY >= bcY && kongkaY <= bcY+128))
            {
                kongkaX=rand() % 780+250;
                kongkaY = 720;
                point--;
...
void jamil()
{
    if(game == 1)
    {
        if(jamilFlag==true)
        {
            ...
            else if((jamilX >= bcX-10 && jamilX <= bcX+128)&&(jamilY >= bcY && jamilY <= bcY+128))
            {
                jamilX=rand() % 780+250;
                jamilY = 720;
                point--;
...
int main()
{
    ...
    //else if
    iInitialize(screenWidth,screenHeight, "M o t i m i a   V   B o r o c h a c h a");
    ...
    leaderboardbg=iLoadImage("images\\leaderboardbg.jpg");
    iSetTimer(time1, mm1);
    iSetTimer(time1, mm2);
    iSetTimer(time1, ab);
